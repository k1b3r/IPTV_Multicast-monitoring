...
#----------start from require id if(if > 100)
if($id > 1000){
goto ERROR;
}else{
warn "$name";
...
$sock = IO::Socket::Multicast->new(Proto=>'udp',LocalAddr=> $link,LocalPort => $port,ReuseAddr => 1,ReusePort => 1,TimeOut => 7) o
r print "Error Open Socket";
